...
delay
    PUSH  { R4 }       // only use R4, so store R4 into the stack
    MOV   R4, R0       // copy R0 to R4
loop
    CBZ   R4, loopexit // compare and branch if zero
                       // if R4 is eqaul to zero, then jump to loopexit
    SUB   R4, R4, #1   // subtract R4 by 1
    B     loop         // jump back to loop label to continue commparing and
                       // decrementing
loopexit
